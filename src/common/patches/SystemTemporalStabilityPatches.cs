...
namespace pl3xtweaks.common.patches;

public sealed class SystemTemporalStabilityPatches {
    internal SystemTemporalStabilityPatches(Harmony harmony) {
        _ = new NoSurfaceInstabilityPatch(harmony);
    }

    private class NoSurfaceInstabilityPatch : AbstractPatch {
        public NoSurfaceInstabilityPatch(Harmony harmony) : base(harmony) {
            Patch<SystemTemporalStability>("GetTemporalStability", Prefix, Postfix,
                types: new[] { typeof(double), typeof(double), typeof(double) });
        }

        [SuppressMessage("ReSharper", "InconsistentNaming")]
        [SuppressMessage("ReSharper", "MemberCanBePrivate.Local")]
        public static bool Prefix(SystemTemporalStability __instance, double x, double y, double z, ref float __result, out float __state, ICoreAPI ___api) {
            __state = 1.0F;
